    ...
    */
    ...
    if(!memcmp(tok.p, "GGA", 3)){
    	...
    	if (r->callback.location_cb) {
			D("GGA: location_cb report: r->fix.flags=%d, r->latitude=%f, r->longitude=%f, r->altitude=%f,"
					" r->speed=%f,r->bearing=%f,r->accuracy=%f\n", r->fix.flags, r->fix.latitude,
					r->fix.longitude, r->fix.altitude, r->fix.speed, r->fix.bearing, r->fix.accuracy);
			r->callback.location_cb(&r->fix);
			r->fix.flags = 0;
		}
		...
		if(tok_fixStatus.p[0] != '\0' && tok_fixStatus.p[0] != '1') {
			...
			nmea_reader_update_accuracy(r, tok_accuracy);
			unsigned int old_mask = r->sv_status.used_in_fix_mask;
			...
			for (i = 3; i <= 14; ++i){
				...
				if (prn > 0){
					...
					// D("GSA, prn=[%d], fix_mask=[%d]", prn, r->sv_status.used_in_fix_mask);
					if(r->sv_status.used_in_fix_mask){
						r->sv_status_changed = 1;
					}else{
						r->sv_status.used_in_fix_mask = old_mask;
					}
				}
			}
			D("GSA: used_in_fix_mask=0x%x", r->sv_status.used_in_fix_mask);           
		}
		D("GSA: fix.flags=0x%x", r->fix.flags);
		...
		if (noSatellites > 0) {
			...
			while (i < 4 && r->sv_status.num_svs < noSatellites){
				...
				if(!r->sv_status.sv_list[curr].prn)
					break;
			...
			if (sentence == totalSentences) {
				// D("msg=%d, msgindex=%d, curr=%d\n", totalSentences, sentence, curr);
				if (r->sv_status_changed != 0) {
					if (r->callback.sv_status_cb) {
#if 0                          
						D("GSV: sv_status_changed=%d\n", r->sv_status_changed);
...
#endif
						D("GSV: sv_status_cb report: num_svs=%d, emask=0x%x, amask=0x%x, iumask=0x%x\n", r->sv_status.num_svs, 
								r->sv_status.ephemeris_mask, r->sv_status.almanac_mask, r->sv_status.used_in_fix_mask);
						r->callback.sv_status_cb(&(r->sv_status));
						r->sv_status.used_in_fix_mask = 0ul;
						r->sv_status_changed = 0;
					}else {
						D("GSV: no callback, keeping status data until needed!");
					}
				}
			}
        ...
        if(tok_fixStatus.p[0] == 'A'){
			...
			if (r->callback.status_cb) {
				D("RMC: status_cb report: fix.flags=%d\n",r->fix.flags);
				r->callback.status_cb((struct GpsStatus *)&(r->fix.flags));
			}
			if (r->callback.location_cb) {
				D("RMC: location_cb report: r->fix.flags=%d, r->latitude=%f, r->longitude=%f, r->altitude=%f,"
						" r->speed=%f,r->bearing=%f,r->accuracy=%f\n", r->fix.flags, r->fix.latitude,
						r->fix.longitude, r->fix.altitude, r->fix.speed, r->fix.bearing, r->fix.accuracy);
				r->callback.location_cb(&r->fix);
				r->fix.flags = 0;
    ...
    if(r->fix.flags != 0){
       ...
       /*
		if(r->callback.location_cb){
			D("location_cb report: r->fix.flags=%d, r->latitude=%f, r->longitude=%f, r->altitude=%f,"
					" r->speed=%f,r->bearing=%f,r->accuracy=%f\n", r->fix.flags, r->fix.latitude,
					r->fix.longitude, r->fix.altitude, r->fix.speed, r->fix.bearing, r->fix.accuracy);
            r->callback.location_cb(&r->fix);
            r->fix.flags = 0;
	...
	// Always report nmea data
	if (r->callback.nmea_cb) {
		D("nmea_cb report: timestamp=%llx, %llu\n",r->fix.timestamp, r->fix.timestamp);
...
int serial_init()
{
	...
	// serial_fd = open("/dev/ttyS0", O_RDONLY|O_NOCTTY|O_NDELAY);
	serial_fd = open("/dev/ttyS3", O_RDONLY | O_NONBLOCK);
	if(serial_fd < 0){
        printf("gps serial open fail\n");
		exit(-1);
	}

	tcgetattr(serial_fd, &termios_old);
	cfsetispeed(&termios_old, B115200);
    ...
    for(;;){
        ...
        for(ne = 0; ne < nevents; ne++){
            if((events[ne].events & (EPOLLERR | EPOLLHUP)) != 0){
				...
				if(gps_fd == events[ne].data.fd){
					epoll_deregister(epoll_fd, gps_fd);
					close(gps_fd);
					// state->fd = socket_local_client(QEMU_CHANNEL_NAME, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_SEQPACKET);
					state->fd = serial_init();
					if(state->fd < 0){
						D("no gps serial port detected, reconnect fail!!");
						return;
					}
					D("reconnected...");
					gps_fd = state->fd;
...
gps_state_init(GpsState* state, GpsCallbacks* callbacks)
{
    ...
    if(state->fd < 0){
        D("no gps serial port detected");
        return;
    }

    D("gps serial port init ok");
...
qemu_gps_init(GpsCallbacks* callbacks)
{
	...
	// system("echo 1 > /sys/class/gpio/gpio210/value");
    return 0;
}
...
qemu_gps_cleanup(void)
{
    ...
    if(s->init)
        gps_state_done(s);
    D("%s: called", __FUNCTION__);
	// system("echo 0 > /sys/class/gpio/gpio210/value");
	return ;
}
...
static int
qemu_gps_set_position_mode(GpsPositionMode mode, GpsPositionRecurrence recurrence, 
